import { CAPABILITIES, escapeHtml, financeValues, has, hasSection, state, statusBadge } from "./state.js?v=20260907-shipment-mixed-quantity-v1";
...
export function renderSupplierWorkspace(supplierId) {
  ...
  return `<section class="supplier-workspace" aria-label="${escapeHtml(supplier.name)} tedarikçi çalışma alanı">
    <header class="supplier-workspace__header supplier-workspace__header--actions"><div class="supplier-workspace__actions">${has(CAPABILITIES.supplierManage) || has(CAPABILITIES.links) ? '<button class="ui-button ui-button--secondary supplier-stock-add-button" type="button" data-action="supplier-add-stock-products"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="m12 3-9 5 9 5 9-5-9-5ZM3 8v9l9 5 4-2M12 13v9M7.5 5.5l9 5M19 13v8M15 17h8"/></svg><span>Stoktan Ekle</span></button>' : ""}<button class="ui-button ui-button--secondary" type="button" data-action="supplier-shipment-history">Sevkiyat Geçmişi</button>${hasSection("ledger", "operate") && (has(CAPABILITIES.accountingPost) || has(CAPABILITIES.paymentCreate)) ? '<button class="ui-button ui-button--secondary" type="button" data-action="supplier-cari">Cari İşle</button>' : ""}${has(CAPABILITIES.supplierManage) || has(CAPABILITIES.links) ? '<button class="ui-button ui-button--primary" type="button" data-action="supplier-add-product">Ürün Ekle</button>' : ""}${has(CAPABILITIES.receiptCreate) ? '<button class="ui-button ui-button--secondary" type="button" data-action="supplier-create-shipment">Sevkiyat Oluştur</button>' : ""}<details class="supplier-secondary-menu"><summary aria-label="Tedarikçi işlemleri">•••</summary><div>${canManageSuppliers() ? '<button type="button" data-supplier-workspace-action="edit">Tedarikçiyi düzenle</button><button class="is-danger" type="button" data-supplier-workspace-action="delete">Tedarikçiyi Sil</button>' : ""}${has(CAPABILITIES.accountingRead) ? '<button type="button" data-supplier-workspace-action="ledger">Cari hesabı aç</button>' : ""}</div></details></div></header>
    ${has(CAPABILITIES.accountingRead) ? financeValues(supplier, "finance-summary supplier-finance-summary") : ""}
    <section class="supplier-products"><header><div><h3>Ürün Kalemleri</h3><p>${products.filter((item) => item.active !== false).length} aktif ürün</p></div></header>
      ${products.length ? `<div class="supplier-product-list">${products.map(supplierProductRow).join("")}</div>` : empty("Ürün kaydı bulunmuyor", "Stoktaki mevcut ürünleri toplu ekleyin veya yeni bir ürün oluşturun.")}
    </section>
  </section>`;
}
