...
function supplierProductRow(item) {
  ...
  return `<article class="supplier-product-row ${item.active === false ? "is-passive" : ""}"><div><strong>${escapeHtml(item.name || "Ürün")}</strong><small>${escapeHtml(item.documentType || "Belge türü belirtilmedi")} · ${item.stockProductId ? "Stokla eşleşti" : "Stok eşleşmesi bekliyor"}</small></div><div><span>Toplu birim</span><strong>${escapeHtml(bulk)}</strong></div><div><span>Temel birim</span><strong>${escapeHtml(base)}</strong></div><div><span>Dönüşüm</span><strong>1 ${escapeHtml(bulk)} = ${Number(item.conversionFactor || 1)} ${escapeHtml(base)}</strong></div><div class="supplier-product-row__actions">${editable ? `<button class="row-button" type="button" data-edit-independent-product="${escapeHtml(item.id)}">Düzenle</button>${item.active !== false ? `<button class="row-button is-danger" type="button" data-deactivate-independent-product="${escapeHtml(item.id)}">Sil</button>` : ""}` : ""}</div></article>`;
}
