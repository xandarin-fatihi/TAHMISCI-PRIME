import { CAPABILITIES, escapeHtml, has, state, statusBadge, trDate, trMoney } from "./state.js";
...
export function shipmentLine(products, index) {
  return `<div class="shipment-line" data-line-index="${index}"><label>Stok ürünü<select name="stockProductId" required><option value="">Ürün seçin</option>${products.map((item) => `<option value="${escapeHtml(item.id)}" data-unit="${escapeHtml(item.unit)}">${escapeHtml(item.name)} · ${escapeHtml(item.stockQuantity)} ${escapeHtml(item.unit)}</option>`).join("")}</select></label><label>Miktar<input name="quantity" type="number" min="0.001" step="0.001" required></label><label>Birim<input name="unit" maxlength="40" required></label><label>Birim fiyat (₺)<input name="unitPrice" type="number" min="0" step="0.01" value="0"></label><button class="remove-line" type="button" data-action="remove-shipment-line" aria-label="Satırı kaldır">×</button></div>`;
}
